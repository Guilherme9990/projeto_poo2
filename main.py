...
class SistemaAcademico:
    def __init__(self, root):
        ...
        self.abas.add(self.aba_alunos, text="Alunos")
        ...
        # Botão de atualização de Registros
        ...
        # Botão de deletar registros
        self.btn_deletar = ttk.Button(self.aba_registros, text="Deletar", command=self.deletar_registro)
    ...
    def carregar_registros(self):
        ...
        for row in cursor.fetchall():
            self.tree.insert("", "end", values=("Aluno", row[0], row[1]))

        # Carregar registros de professores
        cursor.execute("SELECT nome, email FROM Professores")
        for row in cursor.fetchall():
            self.tree.insert("", "end", values=("Professor", row[0], row[1]))
        ...
        """)
        for row in cursor.fetchall():
            self.tree.insert("", "end", values=("Curso", row[0], row[1]))
    ...
    def deletar_registro(self):
        ...
        tipo, nome, *_ = self.tree.item(item_selecionado, "values")
        conn = sqlite3.connect('academico.db')
        cursor = conn.cursor()
        ...
        if tipo == "Aluno":
            ...
            cursor.execute("DELETE FROM Professores WHERE nome = ?", (nome,))
        elif tipo == "Curso":
        ...
        messagebox.showinfo("Sucesso", f"Registro '{nome}' deletado com sucesso!")
